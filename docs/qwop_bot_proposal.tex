...
Our approach to the problem is to develop a neural network and use a genetic algorithm and reinforcement learning with it.

The neural network will be given a set of inputs, which involves the positions of all the limbs in the environment. The goal is to create a deep enough neural network with enough layers, so we can train it using a large dataset of inputs and supervised learning.

To optimize the weights of the neural network we will use a genetic algorithm. The approach that we will use is population-based. In such an approach, we generate multiple agents with various neural networks, which have randomly assigned weights for the first time, and as they evolve the weights will change based on the successful performances of the neural networks.

Finally, to further improve the agent's performance we will use reinforcement learning. For this portion of the development, we will develop a utility function that would indicate how well the agent performs to make further updates to the neural network's weights based on the rewards received by the agent during gameplay. The simplest version of the utility function that we have in mind is to give rewards if the figure moves forward and penalize it for falling or moving backward. We will potentially use a replay buffer to store the outputs of the utility function (agent's experiences) for further improvements in the stability of the learning process.
